	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  // keee
  // "cp" can be any cache. allocate based on "dl1" cache
  int i;
  if(set_miss_counter == NULL && strcmp(cp->name, "dl1") == 0){
	  set_miss_counter = malloc(sizeof(int) * cp->nsets);
	  nsets = cp->nsets;
	  for(i = 0; i < nsets; i++)
		  set_miss_counter[i] = 0;
