	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  // keee 
  // note: "md_addr_t" is "word_t" which is "unsigned int"
 // static int *set_miss_counter;
  if(set_miss_counter == NULL)
	  set_miss_counter = malloc(cp->nsets * sizeof(int));
  ...
  if(strcmp(cp->name, "dl1") == 0){
	  set_miss_counter[set]++;
//	  printf("\nNo.%d miss occurred @ Line %d (out of %d)", cp->misses, set, set_miss_counter[set]);
//	printf("++++ %d\n", set_miss_counter[0]);
