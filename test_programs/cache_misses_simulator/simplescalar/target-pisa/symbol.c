...
/* sudiptac: support to include cache misses only in the 
 * user code */
...
/* sudiptac:::: tell from the name of the function whether it is a 
 * library function, if so, it means no subsequent user functions */
static int
...
/* sudiptac: get start/end addresses of the program by excluding library code */
void
get_prog_range()
{
				int	i, size = 0;
	     ...
	     int load_locals)	/* load local symbols */
{
  ...
  syms_loaded = TRUE;
	...
	/* sudiptac :::: set the effective code range for this program */
	get_prog_range();		  
}
