...
void des(immense inp, immense key, int * newkey, int isw, immense * out) {
   ...
   if (*newkey) {
      *newkey=0;
      ...
      for (j=28,k=56;j>=1;j--,k--) {
		   add_j = getbit(key, ipc1[j], 32);
		   add_k = getbit(key, ipc1[k], 32);
         icd.r = (icd.r <<= 1) | add_j;
         icd.l = (icd.l <<= 1) | add_k;
         }
   ...
   for (j=32,k=64;j>=1;j--,k--) {
		add_j = getbit(key, ip[j], 32);
		add_k = getbit(key, ip[k], 32);
      itmp.r = (itmp.r <<= 1) | add_j;
      itmp.l = (itmp.l <<= 1) | add_k;
      }
   for (i=1;i<=16;i++) {
      ii = (isw == 1 ? 17-i : i);
