...
/* sudiptac: introduced for counting user-code cache misses */
extern md_addr_t start_addr, end_addr;
extern unsigned entry_main;
extern unsigned effect_load_misses;
extern unsigned effect_store_misses;
extern unsigned effect_data_access;

/* sudiptac: support to log stack pointer, global pointer and base pointer 
 * in user code */
extern md_addr_t stack_ptr, glob_ptr, frame_ptr;
...
"      -cache:dl1 ul1:256:32:1:l -cache:dl2 ul2:1024:64:2:l\n"
  ...
  opt_reg_flag(odb, "-bugcompat",
	...
	/* sudiptac: introduced to define the entry point of "main" function 
	 * (optional parameter) */ 
	opt_reg_uint(odb, "-entry:main", "entry point of main function",
	       &entry_main, /* default */0,
	       /* print */TRUE, /* format */NULL);
}
		  ...
		  if (cache_dl1)
		    {
		      /* commit store value to D-cache */
					counter_t misses = cache_dl1->misses;
		      lat =
		      ...
		      if (lat > cache_dl1_lat)
					...
					/* sudiptac: *hack* : account store misses only in the user code */
					if ( (rs->PC >= start_addr) && (rs->PC < end_addr) ) {
						effect_data_access++;
						if (cache_dl1->misses > misses) {
							effect_store_misses++;
#ifdef _NDEBUG
							printf("PC = %x and miss\n", rs->PC);
							fflush(stdout);
#endif
						} else {
#ifdef _NDEBUG
...
ruu_issue(void)
{
  ...
  struct res_template *fu;
	/* sudiptac: memorize the current number of cache misses */
	counter_t misses = cache_dl1->misses;
		  ...
		  if (fu)
			      ...
			      == (F_MEM|F_LOAD)))
			{
			  ...
			  if (!load_lat)
				...
				/* sudiptac: *hack* : account load misses only in the user code */
				if ( (rs->PC >= start_addr) && (rs->PC < end_addr) ) {
					effect_data_access++; 
					if (cache_dl1->misses > misses) {
						effect_load_misses++;
  ...
  for (i=0, branch_cnt=0;
	  ...
	  && !(fetch_regs_PC & (sizeof(md_inst_t)-1)))
	{
	  ...
	  MD_FETCH_INST(inst, mem, fetch_regs_PC);
		...
		/* sudiptac: support to log base pointer, stack pointer and global pointer 
		 * in user code (at the entry of main function) */
		if (fetch_regs_PC == entry_main) {
			stack_ptr = regs.regs_R[29];
			glob_ptr = regs.regs_R[28];
			frame_ptr = regs.regs_R[30];
...
sim_main(void)
{
  ...
  regs.regs_NPC = regs.regs_PC + sizeof(md_inst_t);
	...
	/* sudiptac: load symbols here (support extracting user-only code 
	 * and related cache misses) */
