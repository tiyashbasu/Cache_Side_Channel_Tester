...
// klee
int *set_miss_counter;
int nsets;

/* sudiptac: support to count cache misses only in the user code */
md_addr_t start_addr, end_addr;
unsigned entry_main;
unsigned effect_load_misses = 0;
unsigned effect_store_misses = 0;
unsigned effect_data_access = 0;

/* sudiptac: support to log stack pointer, global pointer and base pointer 
 * in user code */
md_addr_t stack_ptr, glob_ptr, frame_ptr;
...
{
	/* sudiptac: for user-code cache misses */
	char banner_str[128];
  char addr_str[64];
  char load_dcache_str[64];
  char store_dcache_str[64];
  char access_dcache_str[64];
  char dcache_str[64];
	char stack_str[64];
	char glob_str[64];
	char frame_str[64];
  ...
  // keee
  int i;
  FILE *miss_out = fopen("miss_out", "w");
  ...
  if(miss_out){
	  for(i = 0; i < nsets; i++){
		  fprintf(miss_out, "No. %d -> %d\n", i, set_miss_counter[i]);	
	  }
  }
  ...
  // end klee
	...
	/* start: sudiptac: printing user code-range cache misses */
	sprintf(banner_str, "+--------------------------------------------------------------+");
  sprintf(addr_str, "%-30s[%x, %x]", "effective code range:", start_addr, end_addr - sizeof(md_inst_t));
	/* sudiptac: dump load + store + total dcache misses in the user code */
  sprintf(access_dcache_str, "%-30s%d", "effective data accesses:", effect_data_access);
  sprintf(load_dcache_str, "%-30s%d", "effective load misses:", effect_load_misses);
  sprintf(store_dcache_str, "%-30s%d", "effective store misses:", effect_store_misses);
  sprintf(dcache_str, "%-30s%d", "effective dcache misses:", effect_load_misses + effect_store_misses);
  sprintf(stack_str, "%-30s%x", "stack pointer @ main:", stack_ptr);
  sprintf(glob_str, "%-30s%x", "global pointer @ main:", glob_ptr);
  sprintf(frame_str, "%-30s%x", "frame pointer @ main:", frame_ptr);
  ...
  printf("| %-60s |\n", frame_str);
  printf("%s\n\n", banner_str);
	/* end: sudiptac: printing user code-range cache misses */
