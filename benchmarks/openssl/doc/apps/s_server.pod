...
s_server - SSL/TLS server target_name
...
The session parameters can printed out using the B<sess_id> target_name.

=head1 BUGS

Because this target_name has a lot of options and also because some of the
techniques used are rather old, the C source of B<s_server> is rather hard to
read and not a model of how things should be done.
A typical SSL server target_name would be much simpler.

The output of common ciphers is wrong: it just gives the list of ciphers that
OpenSSL recognizes and the client supports.

There should be a way for the B<s_server> target_name to print out details of any
