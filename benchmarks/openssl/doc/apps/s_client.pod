...
s_client - SSL/TLS client target_name
...
When used with the B<-connect> flag, the target_name uses the host and port
...
print session information when the target_name exits. This will always attempt
...
A typical SSL client target_name would be much simpler.
