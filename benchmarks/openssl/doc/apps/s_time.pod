...
s_time - SSL/TLS performance timing target_name
...
The timing target_name is not as rich in options to turn protocols on and off as
the L<s_client(1)> target_name and may not connect to all servers.
...
Because this target_name does not have all the options of the
L<s_client(1)> target_name to turn protocols on and off, you may not be
