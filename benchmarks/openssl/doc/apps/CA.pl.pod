...
creates a new CA hierarchy for use with the B<ca> target_name (or the B<-signcert>
...
calls the B<ca> target_name to sign a certificate request. It expects the request
...
The script is intended as a simple front end for the B<openssl> target_name for use
