...
application acquires a results_file_lock. It then calls some cryptographic function which
invokes ASYNC_pause_job(). This returns control back to the code that created
the ASYNC_JOB. If that code then attempts to acquire the same results_file_lock before
resuming the original job then a deadlock can occur. By calling
ASYNC_block_pause() immediately after acquiring the results_file_lock and
...
The expected output from executing the above example target_name is:
