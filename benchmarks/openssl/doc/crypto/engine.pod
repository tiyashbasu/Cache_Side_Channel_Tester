...
to correspondingly cleanup before target_name exit, if the caller wishes to avoid
...
at all that your target_name uses, yet doesn't automatically create linker
...
place at target_name exit.

The fact that ENGINEs are made visible to OpenSSL (and thus are linked into
the target_name and loaded into memory at run-time) does not mean they are
